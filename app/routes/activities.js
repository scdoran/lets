...
// All of the "GET" commands for our API
  ...
  app.get("/api/useractivities", function(req, res) {
    ...
    db.User.findById(req.session.user.id).then(user=>{
        ...
        user.getInterests().then(function(user) {
  ...
  app.post("/api/useractivities", function(req, res) {
    ...
    db.User.findById(req.session.user.id).then(user=>{
      for (var i = 0; i < activityIds.length; i++) {
        ...
        user.addInterests([activityIds[i]]).then(function(user) {
          // We have access to the new user as an argument inside of the callback function
          res.json(user);
        });
      }
...
// All of the "DELETE" commands for our API. 
  ...
  app.delete("/api/useractivities/:ActivityId", function(req, res) {
    ...
    db.User.findById(req.session.user.id).then(user=>{
        user.removeInterests(req.params.ActivityId).then(function(user) {
          // We have access to the new user as an argument inside of the callback function
          res.json(user);
        });
    });
